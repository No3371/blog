---
layout: post
title: "寫一個自己愛不釋手的工具：Obsidian Regex Pipeline"
tags: foss
published: true
language: tw
---
...
## Obsidian
自從開始使用 [Obsidian](obisidian.md) 以來，也超過半年了。一開始是抱著嘗試新東西的心態，剛好要學日文就拿來收納整理日文筆記，很快就愛上了這東西，一邊學習新知識一邊建立筆記，充實自己的資料庫的感覺，讓我這個數位倉鼠症患者實在是快樂得停不下來，有些下班後的夜晚坐下來編修筆記，轉眼就到了睡覺時間...
...
![](./Obsidian_kHYkCZgx4i.png)

隨便舉個例子，這是我從 goo 辭書上使用 Firefox 的 MardDownload 插件抓下來的[計る](https://dictionary.goo.ne.jp/word/%E8%A8%88%E3%82%8B/)條目的內容：
...
這就是我為了飛速監修而寫的插件 [Regex Pipleline](https://github.com/No3371/obsidian-regex-pipeline)，利用預先定義的 Regex 取代規則，快速調整筆記內容。

## Obsidian 的插件不難寫

Obsidian 的插件 API 很不錯！其實我不常做這種事情，JS/TS 也算是滿生疏的一個領域，但仔細觀察了一下之後還是很容易上手了。個人心得是不像一般是一般為了推出 API 而推出的 API，實際使用上，感覺 Obsidian 官方有認真在思考，根據開發者可能會有的需求去設計實用方便的 API。

比如，我本來預期，這類型的 API 會提供我「現在選取的文字 Index 開頭&結尾」，但它實際上直接提供了一個 `getSelection()` 可以取得完整的已選取文字，還提供了 `replaceSelection()` 讓你直接替換掉以選取的文字，相關的還有 `somethingSelected()`... 可以說是相當高階且人性化的 API 了，插件作者完全可以 Focus 在實現插件功能。
